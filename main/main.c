...
// ── MQTT broker ───────────────────────────────────
#define MQTT_HOST       "10.40.48.71"
#define MQTT_PORT       1883
#define MQTT_USER       "admin"
#define MQTT_PASS       "01001684495"
...
static dht_data_t dht_read(int pin)
{
    ...
    // Start signal: drive low 20 ms (RTOS yields here)
    DHT_OUTPUT(pin);
    DHT_LOW(pin);
    vTaskDelay(pdMS_TO_TICKS(20));

    // Critical section: everything below is µs-precise
    portDISABLE_INTERRUPTS();
    DHT_HIGH(pin);
    esp_rom_delay_us(40);
    DHT_INPUT(pin);   // release bus — pull-up already configured

    int t = 0;
    // Wait for DHT to pull low (~80 µs)
    while (DHT_READ(pin) == 1) { esp_rom_delay_us(1); if (++t > 200) goto done; }
    t = 0;
    // Wait for DHT to release high (~80 µs)
    while (DHT_READ(pin) == 0) { esp_rom_delay_us(1); if (++t > 200) goto done; }
    t = 0;
    // Wait for DHT to pull low again (start of bit 0)
    while (DHT_READ(pin) == 1) { esp_rom_delay_us(1); if (++t > 200) goto done; }
    ...
    for (int i = 0; i < 40; i++) {
        ...
        // Sample at 35 µs: still high → '1' (70 µs pulse), already low → '0' (26 µs pulse)
        esp_rom_delay_us(35);
...
// Encode MQTT variable-length integer, return number of bytes written
...
// Read one complete MQTT packet; returns upper-nibble packet type or -1 on error
...
static bool mqtt_send_connect_pkt(void)
{
    ...
    pl[plen++] = 0x04;   // MQTT 3.1.1
    pl[plen++] = 0xC2;   // clean session | username | password
...
// Public: publish from any task (thread-safe via mutex)
...
// ── Receive task: runs until socket error, then exits so main can reconnect ───
static void mqtt_recv_task(void *arg)
{
    ...
    while (1) {
        ...
        if (type == 0x30 && pkt_len >= 2) {  // PUBLISH
        ...
        // SUBACK (0x90) and PINGRESP (0xD0) are silently accepted
    }
...
// ── Open TCP connection and perform MQTT handshake ────────────────────────────
static bool mqtt_connect(void)
{
    ...
    // Expect CONNACK (4 bytes: 0x20 0x02 flags rc)
    uint8_t ack[4] = {0};
...
void app_main(void)
{
    ...
    // Configure DHT22 pull-up once; direct-register dht_read() preserves it
    ...
    while (1) {
        ...
        // ── PIR ──────────────────────────────────────────────────────────────
        room_occupied = (gpio_get_level(PIR_PIN) == 1);
        if (room_occupied && !last_occupied) {
            ESP_LOGI(TAG, ">>> SOMEONE ENTERED");
            mqtt_pub(TOPIC_ROOM, "OCCUPIED");
        } else if (!room_occupied && last_occupied) {
            ESP_LOGI(TAG, "<<< ROOM EMPTY");
            mqtt_pub(TOPIC_ROOM, "EMPTY");
        }
        last_occupied = room_occupied;
